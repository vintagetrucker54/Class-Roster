...
void Roster::parseAdd(string row)
{
	...
	lhs = rhs + 1;
	rhs = row.find(",", lhs);
	int courseDays0 = stoi(row.substr(lhs, rhs - lhs));

	lhs = rhs + 1;
	rhs = row.find(",", lhs);
	int courseDays1 = stoi(row.substr(lhs, rhs - lhs));

	lhs = rhs + 1;
	rhs = row.find(",", lhs);
	int courseDays2 = stoi(row.substr(lhs, rhs - lhs));
	...
	else { degreeProgram = Degree::UNKNOWN; }
